...
const getRelatedPosts = (currentId: string, category: string) => {
  return blogPosts.filter((p) => p.category === category && p.id !== currentId).slice(0, 3)
}
        ...
        <div className="border-t pt-8">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {relatedPosts.map((relatedPost) => (
              <Link key={relatedPost.id} href={`/dashboard/content/blog/${relatedPost.id}`} className="block h-full">
